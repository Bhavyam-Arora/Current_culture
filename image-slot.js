...
(() => {
  // When bundled into a standalone HTML via the inliner, the host page
  // exposes the pre-fetched sidecar at window.__resources.imageSlotState
  // (a blob: URL). Fall back to the on-disk file otherwise.
  const STATE_FILE = (typeof window !== 'undefined'
    && window.__resources
    && window.__resources.imageSlotState) || '.image-slots.state.json';
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      this.addEventListener('dblclick', (e) => {
        if (!this.hasAttribute('data-editable') || !this._reframes() || this.hasAttribute('locked')) return;
        e.preventDefault();
